...
//Se non e' presente il file di configurazione il programma fallisce
	if (configuration_file_pointer == NULL) {fprintf(stderr, "Error, I cannot open DigitizerConfig.conf\n"); exit(1);}
...
//Apro il digitizer con le informazioni ricavate, se non e' possibile aprire il digitizer il programma termina.
	...
	if (ret != CAEN_DGTZ_Success)	{fprintf(stderr, "Error: cannot open the digitizer\n"); exit(1);}
...
//Apro il file contenente i dati da decodificare, quello contenente le dimensioni dei dati da modificare e quello che conterra' i risultati della decodifica.
	...
	if (datirozzi == NULL || sizedatirozzi == NULL || eventodecodificato == NULL) {fprintf(stderr, "I can't open specified files\n"); exit(1);}
...
*/
	...
	if (ret != CAEN_DGTZ_Success)	{fprintf(stderr, "Error: cannot get board info\n"); exit(1);}
